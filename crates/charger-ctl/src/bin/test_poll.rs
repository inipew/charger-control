...
fn main() {
    ...
    if fd < 0 {
        eprintln!("[-] Gagal membuat socket netlink: {}", std::io::Error::last_os_error());
        std::process::exit(1);
    }
    ...
    if ret < 0 {
        eprintln!("[-] Gagal bind socket netlink: {}", std::io::Error::last_os_error());
        std::process::exit(1);
    }
            ...
            if len > 0 {
                ...
                if s.contains("SUBSYSTEM=power_supply") {
                    ...
                    for part in s.split('\0').filter(|&x| !x.is_empty()) {
                        if part.starts_with("ACTION=") || part.starts_with("SUBSYSTEM=") || part.starts_with("POWER_SUPPLY_NAME=") || part.starts_with("POWER_SUPPLY_STATUS=") {
                            println!("    -> {}", part);
                        }
                    }
